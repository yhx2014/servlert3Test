...
//密码验证
jQuery.validator.addMethod("passwords",function(value, element, param) {
	return value = $(param).val();
},"两次密码输出不相同，请重新设置");
...
jQuery.validator.addMethod("charLength", function(value, element) {
    var length = value.length;   
    return this.optional(element) || (length == 6 && /^(?![a-zA-Z]+$)(?![0-9]+$)[a-zA-Z0-9]{6}$/.test(value));
}, "编码长度必须为6须包含字母和数字");
jQuery.validator.addMethod("menuurl", function(value, element) {
	var length = value.startsWith("/");   
	return this.optional(element) || (value.startsWith("/"));
}, "菜单地址不合法");
jQuery.validator.addMethod("charAuthLength", function(value, element) {
    var length = value.length;   
    return this.optional(element) || (length >= 6 && /^(?![a-zA-Z]+$)(?![0-9]+$)[a-zA-Z0-9]{6,}$/.test(value));
}, "授权号长度必须为6位以上须包含字母和数字");

jQuery.validator.addMethod("numberLength", function(value, element) {
    var length = value.length;   
    return this.optional(element) ||(length == 4 && /^([0-9])\d{3}$/.test(value));
}, "POS编码长度必须长度为4的数字");
...
jQuery.validator.addMethod("decmal1", function(value, element) {
    var decmal1 = /^[0-9]+(\.[0-9]+)?$/;
    return this.optional(element) || (decmal1.test(value));
}, "请输入正确数字");
